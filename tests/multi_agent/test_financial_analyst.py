...
- Tool filtering (web_search, calculator, perplexity_search)
...
from src.multi_agent.specialists.financial_analyst import FinancialAnalystAgent
...
class TestFinancialAnalystAgentInitialization:
    ...
    def test_agent_type_is_financial_analyst(self):
        """Agent should have FINANCIAL_ANALYST type."""
    ...
    def test_agent_name_exists(self):
        ...
        with patch('anthropic.Anthropic'):
            ...
            assert "financial" in agent.agent_name.lower() or "analyst" in agent.agent_name.lower()
    ...
    def test_allowed_tools_from_matrix(self):
        ...
        with patch('anthropic.Anthropic'):
            ...
            expected_tools = ToolAccessMatrix.get_allowed_tools(AgentType.FINANCIAL_ANALYST)
...
class TestFinancialAnalystAgentToolFiltering:
    """Tests for FinancialAnalystAgent tool filtering."""
    ...
    def test_filtered_tools_only_includes_allowed(self):
        """Filtered tools should only include web_search, calculator, perplexity_search."""
        with patch('anthropic.Anthropic'):
            agent = FinancialAnalystAgent(
                tool_registry=self._create_mock_registry()
            )
...
class TestFinancialAnalystAgentSystemPrompt:
    """Tests for FinancialAnalystAgent system prompt."""
    ...
    def test_system_prompt_mentions_financial(self):
        """System prompt should mention financial domain."""
        with patch('anthropic.Anthropic'):
            agent = FinancialAnalystAgent(
                tool_registry=self._create_mock_registry()
            )
            prompt_lower = agent.system_prompt.lower()
            assert "financial" in prompt_lower or "finance" in prompt_lower

    def test_system_prompt_mentions_calculation(self):
        """System prompt should mention calculations or numbers."""
        with patch('anthropic.Anthropic'):
            agent = FinancialAnalystAgent(
                tool_registry=self._create_mock_registry()
            )
            prompt_lower = agent.system_prompt.lower()
            has_calc_term = any(term in prompt_lower for term in [
                "calculat", "numeric", "number", "ratio", "percent",
                "market cap", "revenue", "growth", "p/e", "valuation"
            ])
            assert has_calc_term

    def test_system_prompt_mentions_data_freshness(self):
        """System prompt should emphasize data freshness or accuracy."""
        with patch('anthropic.Anthropic'):
            agent = FinancialAnalystAgent(
                tool_registry=self._create_mock_registry()
            )
            prompt_lower = agent.system_prompt.lower()
            has_freshness_term = any(term in prompt_lower for term in [
                "fresh", "current", "recent", "latest", "up-to-date",
                "accurate", "precise", "date", "source"
            ])
            assert has_freshness_term


class TestFinancialAnalystAgentExecution:
    """Tests for FinancialAnalystAgent execution flow."""
    ...
    def _create_mock_registry(self):
        ...
        mock_registry.execute_tool.return_value = {
            "success": True,
            "result": "Apple market cap: $3 trillion",
        }
        return mock_registry
    ...
    async def test_execute_returns_agent_response(self):
        ...
        mock_response.content = [MagicMock(text="Apple's market cap is approximately $3 trillion.")]
        ...
        with patch('anthropic.Anthropic') as mock_anthropic:
            ...
            result = await agent.execute("What is Apple's market cap?")

            assert isinstance(result, AgentResponse)
            assert result.agent_type == AgentType.FINANCIAL_ANALYST
    ...
    async def test_execute_includes_query(self):
        ...
        mock_response = MagicMock()
        mock_response.stop_reason = "end_turn"
        mock_response.content = [MagicMock(text="Tesla P/E ratio is 50.")]
        mock_response.content[0].type = "text"
        ...
        with patch('anthropic.Anthropic') as mock_anthropic:
            ...
            result = await agent.execute("What is Tesla's P/E ratio?")

            assert result.query == "What is Tesla's P/E ratio?"
    ...
    async def test_execute_handles_errors(self):
        ...
        with patch('anthropic.Anthropic') as mock_anthropic:
            ...
            result = await agent.execute("What is NVIDIA's revenue?")
